...
Press `win` to search PowerShell and open it.
按 `win` 搜素 PowerShell 并打开。
...
Type `brew install --cask claude-code` in the window and press `Enter`. Once the process is completed, the installation will be successfully completed.
在打开的窗口中输入 `brew install --cask claude-code` ，按下 `Enter` 。等待完成即可成功安装claude code.
...
Enter claude in terminal and you can open the window of claude code.
在 terminal 输入 claude 即可打开 claude code 的窗口。

![](./images/Toolkit/image36.png)

Here's a simple introduction to claude code window's elements. In the following part, we'll introduce them detailly.
下面是对claude code窗口元素的简单介绍。在下面的章节，我们会详细介绍他们。
...
Note：Think of the **context window** as a sliding window: once the
**token** limit is reached, the model automatically "pushes out" the earliest memories to make room. If Claude starts forgetting instructions or repeating addressed issues, it usually means the critical context has fallen out of range. In such cases, use **/compress** to streamline your context.
注：**上下文 Token 数**就像一个滑动窗口：一旦达到上限，模型会通过挤掉最早的记忆来腾出空间。如果你发现 Claude 开始遗忘指令或重复已解决的问题，通常是因为关键上下文已超出窗口范围。此时，建议使用 **/compress** 命令来精简并压缩上下文。

![](./images/Toolkit/image48.png)

6. **/clear**

Reset session and clear context.清空重来。
...
**Bash mode**: Directly execute system commands (such as `!pwd`), does not consume AI **tokens**, is fast, and is suitable for file operations, Git management, package installation, etc.

**Bash 模式**：直接执行系统命令（如`!pwd`），不消耗 AI **token**，速度快，适合文件操作、Git 管理、包安装等。

注：pwd为bash命令，会打印当前目录的完整路径。
...
**Command mode**: Calling Claude's built-in functions (such as `/clear` to clear context, `/model` to switch AI models, `/cost` to view **token** consumption, for details, please refer to Section 3.4.3) is the core operation entry point of the tool.
**命令模式**：调用 Claude 内置功能（如`/clear`清空上下文、`/model`切换 AI 模型、`/cost`查看 **token** 消耗，详情可参考3.4.3节），是工具的核心操作入口。

![](./images/Toolkit/image53.png)

3. **@**

**File path mode**: Quickly reference files/directories in the project
(such as `@.zshrc`), facilitating AI to locate code files for analysis/modification.
**文件路径模式**：快速引用项目中的文件 / 目录（如`@.zshrc`），方便 AI 定位代码文件进行分析 / 修改。
...
Claude code provides methods for customizing commands. Specifically, create a reusable md file as a teleprompter in the home directory **`/.claude/commands`** folder. The file name is the name of the command, and then it can be quickly invoked in claude code. This command will reuse the teleprompter in the md and complete the task in the current working directory. The usage method is consistent with Section 3.4.1, and it is invoked using the syntax of the **`/ + command`**.
Claude code提供了自定义命令的方法。具体而言，在家目录 **`/.claude/commands`** 文件夹创建一个作为提词的可重复使用md文件，文件名为命令的名称，然后就可在claude code快速调用，该命令会重复使用md中的提词，在当前的工作目录完成任务。使用方法和3.4.1节一致，使用 **`/ + 命令`** 的语法进行调用。

![](./images/Toolkit/image56.png)

commandernatively, commands can also be added by directly asking claude code to generate custom commands. However, it should be noted that commands added in this way may require restarting the claude code to view the newly added commands. Here is an example of prompt:
...
或者，也可以通过直接要求claude code生成自定义命令的方式添加命令。但是需要注意的是，通过这种方式添加的命令可能需要重启claude code才能看到新添加的命令。以下为提词示例：
