...
>[!NOTE]
>
> 若尚未安装 Homebrew，请先安装 [Homebrew](../../../Basic-tools/05-homebrew-install)。
...
>[!TIP]
>
> VS Code 更适合处理纯文本（如 `.txt`、`.md`、`.html`）；Word、Excel、PPT 这类格式一般要装专门插件或用别的软件打开。
...
>[!TIP]
>
> 装 **Markdown All in One**、**Md Editor** 等插件后，就能直接预览 Markdown 效果，不用自己脑补排版。
...
>[!TIP]
>
> VS Code 可以直接新建 `.txt`、`.md`、`.json` 等纯文本文件；Word、Excel、PPT 这类建议用对应软件创建。
...
>[!TIP]
>
> 在 Mac 上，也可选中文件后按下 Enter 键快速重命名。
...
>[!TIP]
>
> 装 PDF、Office 等格式对应的插件后，就能在 VS Code 里直接看、直接改这些文件，一个应用搞定多种格式。
