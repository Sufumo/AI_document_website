...
>[!TIP]
>
> 按 `Ctrl + Space`（或你设置的自定义快捷键）可以在任何地方调出 Everything。就像拥有了搜索超能力。
...
>[!WARNING]
>
> **开发者必读**：默认情况下，Windows 隐藏文件扩展名（所以 `script.sh` 只显示为 `script`）并隐藏系统文件。当你处理 `.gitignore`、`.env` 等配置文件，或区分 `.json` 和 `.js` 时，这简直是噩梦。
...
>[!TIP]
>
> 现在你能看到 `.gitignore`、`.env` 等文件，每个文件都会显示真实的扩展名。好多了。
